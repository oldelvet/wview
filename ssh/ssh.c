...
static void timerHandler (void *parm)
{
    ...
    int32_t delayoffset = (int32_t) netOffset;
    ...
    if ((timerWait <= 0) || (timerWait > 120000)) {
        radMsgLog (PRI_HIGH, "wviewsshd: unusual wait %d fixing up", timerWait);
        // Keep the offset into the minute to keep updates aligned with the original
        // alignment.
